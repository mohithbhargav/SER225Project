...
public class GuardW extends Enemy {
    ...
    public GuardW(Point startLocation, Point endLocation, Direction facingDirection) {
        super(startLocation.x, startLocation.y, new SpriteSheet(ImageLoader.load("WGuard.png"), 26, 30), "WALK_RIGHT");
    ...
    public void update(Player player) {
        ...
        if (dinosaurState == DinosaurState.SHOOT) {
            ...
            float playerX = player.getCalibratedXLocation();
          
            float movementSpeed;
            float guardX = this.getCalibratedXLocation();
            float fireballX = guardX;
            ...
            if (facingDirection == Direction.RIGHT ) {
                fireballX = + player.getCalibratedXLocation() - getWidth() - 45;
                movementSpeed = 2.5f;
            } else {
               fireballX = player.getCalibratedXLocation() +  getWidth() + 45;
                movementSpeed = -2.5f;
            ...
            float fireballY = getY() + 50;

            // create Fireball enemy
            Fireball fireball = new Fireball(new Point(fireballX, fireballY), movementSpeed, 100);
    ...
    private void moveTowardsPlayer(float playerX) {
        float distanceToMove = movementSpeed;
        if (Math.abs(playerX - getX()) < distanceToMove) {
            distanceToMove = Math.abs(playerX - getX());
        }


        if (facingDirection == Direction.RIGHT) {
            moveXHandleCollision(distanceToMove);
        } else {
            moveXHandleCollision(-distanceToMove);
        }
